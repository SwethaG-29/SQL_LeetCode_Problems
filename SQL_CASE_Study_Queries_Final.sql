...
SELECT COUNT(movie_id) as One_Genre
FROM (
  SELECT movie_id
  FROM genre
  GROUP BY movie_id
...
RANK() OVER (Order by avg_rating desc ) as Ranking_Movie_Rating
...
INNER JOIN ratings AS r
ON m.id=r.movie_id
WHERE r.avg_rating>8 AND g.genre IN (SELECT genre FROM Top_Directors)
...
SELECT n.name AS actress_name,SUM(r.total_votes),COUNT(r.movie_id) as movie_count,
...
FROM movie m
INNER JOIN genre g -- join on movie and genre
ON m.id = g.movie_id
WHERE genre IN(SELECT genre FROM Top_genre) -- pick only top 3 genre movies
)
SELECT *
FROM Top_movie
WHERE movie_rank<=5 -- to find only top 5 movies of each genre
ORDER BY movie_rank;
